...
function Lua_Loader.define(name, moduleFunction)
    ...
    if IsReserved(name) then
        return error("Invalid call to "..methodName..". Given name is reserved '"..type(name).."''")
    end
    ...
    if modules[name] ~= nil then
        error("Invalid call to Lua_Loader.define(). A module already registered with the name '"..name.."''")
    end
